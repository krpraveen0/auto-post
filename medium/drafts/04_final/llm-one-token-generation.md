...
When people talk about an LLM response, they usually picture a prompt going in and an answer coming out. That is a fine mental model if you are just using the API.

It is the wrong mental model if you are trying to ship the system.
...
In production, a request usually passes through a gateway or serving layer first. The system may normalize the prompt, enforce limits, queue it, or batch it with other requests. Only after that does it hit the model.
